...
Nets = []
input_dims = []
file_path = './PPMI/' + org + '_net_{}.mat'

for i in select_nets:
    print(f"### [{i}] Loading network...")
    
    N = sio.loadmat(file_path.format(i), squeeze_me=True)['Net'].todense()
    
    print(f"Net {i}, NNofile_keywords={np.count_nonzero(N)} \n")
    
    Nets.append(minmax_scale(N))
    input_dims.append(N.shape[1])
